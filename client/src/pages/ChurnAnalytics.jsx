  ...
  return (
    <div className="p-4 lg:p-6 space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'High Churn', key: 'High', color: '#FF4F4F' },
          { label: 'Medium Churn', key: 'Medium', color: '#FF6B2B' },
          { label: 'Low Churn', key: 'Low', color: '#22B26F' },
        ].map(({ label, key, color }) => {
          ...
          return (
            <motion.div key={key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              className="rounded-lg p-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderTop: `3px solid ${color}` }}>
              ...
              {/* Mini progress bar */}
              <div className="mt-3 h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--track-bg)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-5 rounded-lg">
          ...
          <div className="space-y-3">
            {(churn || []).map(d => {
              ...
              const color = d.name === 'High' ? '#FF4F4F' : d.name === 'Medium' ? '#FF6B2B' : '#22B26F'
              return (
                <div key={d.name}>
